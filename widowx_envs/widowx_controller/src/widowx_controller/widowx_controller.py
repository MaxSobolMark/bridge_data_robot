...
class WidowX_Controller(RobotControllerBase):
    def __init__(
        ...
        rospy.Subscriber(
            f"/{robot_name}/joint_states", JointState, self._joint_callback
        )
        time.sleep(1)
        self._n_errors = 0
    ...
    def reboot_motor(self, joint_name: str):
        ...
        try:
            ...
            response = reboot_motors(
                cmd_type="single", name=joint_name, enable=True, smart_reboot=True
            )
            return response
        ...
        try:
            ...
            if check_effort:
                if np.max(np.abs(self.get_joint_effort()) - ABS_MAX_JOINT_EFFORTS) > 10:
                    print(
                        "violation ",
                        np.abs(self.get_joint_effort()) - ABS_MAX_JOINT_EFFORTS,
                    )
                    print(
                        "motor number: ",
                        np.argmax(
    ...
    def move_to_neutral(self, duration=4):
        ...
        try:
            ...
            # import ipdb; ipdb.set_trace()
            if (
                np.linalg.norm(self.neutral_joint_angles - self.get_joint_angles())
                > 0.1
            ):
    ...
    def get_gripper_position(self):
        assert (
            self.custom_gripper_controller
        )  # we need the joint_states subscriber to keep track of gripper position
